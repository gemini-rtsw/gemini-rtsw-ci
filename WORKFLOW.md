...
- **From the GitHub Actions run** — the RPM is also uploaded as a workflow artifact named `rpms-el<N>`. Open the run under the repo's **Actions** tab and download it from **Artifacts**, or via the `gh` CLI:
  ```bash
